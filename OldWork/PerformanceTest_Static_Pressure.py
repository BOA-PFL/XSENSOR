...
static_time = 2 # time in seconds to examine static pressures
# list of functions 
...
fPath = 'C:\\Users\\eric.honert\\Boa Technology Inc\\PFL Team - General\\Cycling Performance Tests\\Cycling_4guideSD_Feb2022\XSENSOR Data\\'
fileExt = r".csv"
entries = [fName for fName in os.listdir(fPath) if fName.endswith(fileExt) and fName.find('tatic') > 0]
...
for fName in entries:   
    ...
    try:
            ...
            contact_per.append(np.mean(dat.ContactArea_LF[0:static_time*freq]))
            peak_press.append(np.mean(dat.PeakP_LF[0:static_time*freq]))
            Sub.append( fName.split('_')[0] )
            Config.append(fName.split('_')[1])
            
            contact_per.append(np.mean(dat.ContactArea_RF[0:static_time*freq]))
            peak_press.append(np.mean(dat.PeakP_RF[0:static_time*freq]))
            Sub.append( fName.split('_')[0] )
            Config.append(fName.split('_')[1])
...
Outcomes = pd.DataFrame({ 'Subject':list(Sub),'Config':list(Config),
                    'ContactPerc': list(contact_per), 'PeakPress':list(peak_press)})  
